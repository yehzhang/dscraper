...
def parse_args():
    ...
    parser.add_argument('-m', '--merge', required=False, action='store_true', default=False,
                        help='whether merging comments of different dates as one file if -e "file" was specified')
    ...
    parser.add_argument('-d', '--dates', metavar=('begin', 'end'), nargs=2, required=False, type=int, default=(None, None),
                        help='two unix timestamps specifying the beginning and ending dates between which comments should be scraped (inclusive)')
    parser.add_argument('-r', '--range', metavar=('first', 'last'), required=False, nargs=2, type=int, action='append',
                        default=[], help='the first and last ID numbers of consecutive targets to scrape. Can be specified multiple times')
...
def main():
    ...
    args = parse_args()
    export, path, time_range, mode, range_targets, targets, merge, history = args.export, \
